...
namespace HobbyHorseApi.JsonConverters
{
    public class OutingConverter : System.Text.Json.Serialization.JsonConverter<Outing>
    {
        public override Outing Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            try
            {
                ...
                {
                    //it is a ParkTrail
                    JsonElement locationElement = trailElement.GetProperty("location");
                    trail = new ParkTrail()
                    {
                        ...
                        {
                            Id = locationElement.GetProperty("id").GetString(),
                            Name = locationElement.GetProperty("name").GetString(),
                            ImageUrl = locationElement.GetProperty("imageUrl").GetString(),
                            Lat = locationElement.GetProperty("lat").GetDouble(),
                            Long = locationElement.GetProperty("long").GetDouble()
                        },
